...
public class PermissionChecker {
    ...
    public static Optional<String> checkMemberPermission(BotConfigManager botConfigManager, Member member, Permission action) {
        if (member.isOwner() || member.hasPermission(net.dv8tion.jda.core.Permission.ADMINISTRATOR) || !checkBotAdmin(member)
                .isPresent()) {
            return Optional.empty();
        }
        if (!GuildDataManager.getGuildPermissionSet(botConfigManager, member.getGuild())
                             .checkMemberPermission(member, action)) {
